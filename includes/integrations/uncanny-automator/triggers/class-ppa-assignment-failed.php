<?php
/**
 * Assignment Failed Trigger
 *
 * Fires when a user fails an assignment (score below passing threshold).
 *
 ...
 * @since 1.0.0
 */
class PressPrimer_Assignment_Assignment_Failed extends \Uncanny_Automator\Recipe\Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'PPA_ASSIGNMENT_FAILED' );
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr__( 'A user fails {{an assignment:%1$s}}', 'pressprimer-assignment' ),
				$this->get_trigger_meta()
			)
		);

		$this->set_readable_sentence(
			esc_attr__( 'A user fails {{an assignment}}', 'pressprimer-assignment' )
		);

		// Hook: pressprimer_assignment_submission_failed( $submission_id, $score ).
		$this->add_action( 'pressprimer_assignment_submission_failed', 10, 2 );
	}

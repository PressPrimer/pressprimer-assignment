<?php
/**
 * Assignment Passed Trigger
 *
 * Fires when a user passes an assignment (score meets or exceeds passing threshold).
 *
 ...
 * @since 1.0.0
 */
class PressPrimer_Assignment_Assignment_Passed extends \Uncanny_Automator\Recipe\Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'PPA_ASSIGNMENT_PASSED' );
		...
		$this->set_sentence(
			sprintf(
				...
				esc_attr__( 'A user passes {{an assignment:%1$s}}', 'pressprimer-assignment' ),
				$this->get_trigger_meta()
			)
		);

		$this->set_readable_sentence(
			esc_attr__( 'A user passes {{an assignment}}', 'pressprimer-assignment' )
		);

		// Hook: pressprimer_assignment_submission_passed( $submission_id, $score ).
		$this->add_action( 'pressprimer_assignment_submission_passed', 10, 2 );
	}
